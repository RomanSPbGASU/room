...
{
    if (version_compare(GLPI_VERSION, '9.3.1', '>=') && version_compare(GLPI_VERSION, '9.6', '<')) {
        return true;
    } else {
        if (method_exists('Plugin', 'messageIncompatible')) {
            echo Plugin::messageIncompatible('core', '9.3.1', '9.6');
        } else {
            echo "This plugin requires GLPI >= 9.3.1 && < 9.6";
        }
        return false;
    }
}
